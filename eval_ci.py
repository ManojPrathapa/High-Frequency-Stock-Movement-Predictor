...
def run_sanity_tests(df):
    ...
    assert df['rolling_avg_10'].isnull().sum() == 0, "Null values found in rolling_avg_10!"
    assert df['volume_sum_10'].isnull().sum() == 0, "Null values found in volume_sum_10!"
    assert (df['rolling_avg_10'] > 0).all(), "rolling_avg_10 contains non-positive values!"
    assert (df['volume_sum_10'] >= 0).all(), "volume_sum_10 contains negative values!"
...
def evaluate_and_report():
    ...
    # 4. Generate visual plot for CML Report
    plt.figure(figsize=(6, 4))
    metrics_names = ['Accuracy', 'Precision', 'Recall', 'F1-Score']
    metrics_vals = [acc, prec, rec, f1]
    plt.bar(metrics_names, metrics_vals, color=['blue', 'green', 'orange', 'red'])
    plt.ylim(0, 1.0)
    plt.title("Test Dataset Evaluation Metrics")
    plt.savefig("metrics_plot.png")
    plt.close()
    
    # 5. Generate report.md for CML
    with open("report.md", "w") as f:
        ...
        f.write("![Metrics Plot](./metrics_plot.png)\n")
        
    print("CML Report generated successfully as 'report.md'!")
